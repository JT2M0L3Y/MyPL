...
// expressions
TEST(AddedNegExprTests, MissingOp) {
  stringstream in("void f() { x = 1 2 }");
  try {
    SimpleParser(Lexer(in)).parse();
    FAIL();
  }
...
TEST(AddedNegExprTests, MissingParenInExpr) {
  stringstream in("void f() { x = 1 + (2 * 3 }");
  try {
    SimpleParser(Lexer(in)).parse();
    FAIL();
  }
...
TEST(AddedNegExprTests, MissingExprInNot) {
  stringstream in("void f() { x = not }");
  try {
    SimpleParser(Lexer(in)).parse();
    FAIL();
  }
...
TEST(AddedNegExprTests, MissingLValueInExpr) {
  stringstream in("void f() { x = + 1 }");
  try {
    SimpleParser(Lexer(in)).parse();
    FAIL();
  }
...
TEST(AddedNegExprTests, InvalidNewRValue) {
  stringstream in("void f() { x = new 1 }");
  try {
    SimpleParser(Lexer(in)).parse();
    FAIL();
  }
